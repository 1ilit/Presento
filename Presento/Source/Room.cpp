...
void Room::Update(Player* p) {
	...
	if (input->KeyDown(SDL_SCANCODE_D)) {
		if (collidingRight) {
			...
			p->tracker->Pos(Vector2(p->Pos().x + velocity.x, p->tracker->Pos().y));
		}
	}
	...
	displacement.x = p->tracker->Pos().x - previousPos.x;
	displacement.y = p->tracker->Pos().y - previousPos.y;
}
