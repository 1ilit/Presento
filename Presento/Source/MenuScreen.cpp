...
bool MenuScreen::GameStarted() {
	return play->wasClicked || input->KeyReleased(SDL_SCANCODE_RETURN);
}

void MenuScreen::SetFalse() {
	play->wasClicked = false;
}

bool MenuScreen::WentBack() {
	return back->wasClicked;
}

bool MenuScreen::WentToAch() {
	return achievements->wasClicked;
}

void MenuScreen::SetClicked(bool b) {
	back->wasClicked = b;
}

void MenuScreen::SetAch(bool b) {
